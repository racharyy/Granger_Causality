...
lseq = np.array([ lstotal[k]+np.random.randint(1,7) for k in cat_order])
...
# Make the plot
plt.bar(r1, bars1, color='#87cefa', width=barWidth, edgecolor='white', label='LS')
plt.bar(r2, bars2, color='#ffa07a', width=barWidth, edgecolor='white', label='NLS')
 
# Add xticks on the middle of the group bars
plt.xlabel('Search Categories', fontweight='bold', fontsize=12)
plt.xticks([r + barWidth for r in range(len(bars1))], higher_ls,fontsize=14,rotation=90)
 
# Create legend & Show graphic
plt.legend()
plt.ylabel("% searches",fontweight='bold', fontsize=12)
plt.subplots_adjust(bottom=0.3)
plt.tight_layout()
plt.grid()
plt.show()
